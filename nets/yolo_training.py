...
class YOLOLoss(nn.Module):
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        nwd_loss = (1.0 - nwd).mean() if nwd.numel() > 0 else torch.tensor(1.0, device=nwd.device, requires_grad=True)

        nwd_weight = 0.5
        ciou_weight = 0.5
        loss_iou = nwd_weight * nwd_loss + ciou_weight * loss_iou

        if self.use_focal_loss:
            loss_obj = (self.focal_loss(obj_preds.view(-1, 1), obj_targets)).sum()
            loss_cls = (self.focal_loss(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)).sum()
        else:
        ...
        loss = reg_weight * loss_iou + obj_weight * loss_obj + cls_weight * loss_cls
